...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Messages', href: '/labtechnician/messages' },
];
...
export default function AdminMessages({ users, messages, selectedUserId, search, authUserId }) {
  ...
  function handleSearch(e) {
    e.preventDefault();
    router.get(route('labtechnician.messages.index'), { search: searchValue });
  }

  function handleSelectUser(userId) {
    setSelectedUser(userId);
    setData('receiver_id', userId);
    router.get(route('labtechnician.messages.index'), { user: userId, search: searchValue });
  }

  function handleSend(e) {
    e.preventDefault();
    if (!data.body.trim()) return;
    post(route('labtechnician.messages.send'), {
      preserveScroll: true,
