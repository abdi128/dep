...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Lab Technicians', href: '/admin/labtechnicians' },
];

export default function AdminLabtechnicians({ labtechnicians, filters }) {
  const [openNew, setOpenNew] = useState(false);
  const [openView, setOpenView] = useState(false);
  const [openEdit, setOpenEdit] = useState(false);
  const [selectedLabtech, setSelectedLabtech] = useState(null);

  // New lab technician form
  const {
    data: newData,
    setData: setNewData,
    post,
  ...
  function submitNew(e) {
    e.preventDefault();
    post(route('admin.labtechnicians.store'), {
      onSuccess: () => {
        resetNew();
        setOpenNew(false);
      },
  ...
  function openViewModal(labtech) {
    setSelectedLabtech(labtech);
    setOpenView(true);
  }

  function openEditModal(labtech) {
    setSelectedLabtech(labtech);
    setEditData({
      name: labtech.user?.name || '',
      email: labtech.user?.email || '',
      password: '',
      password_confirmation: '',
      first_name: labtech.first_name,
      last_name: labtech.last_name,
      phone_number: labtech.phone_number,
    });
    setOpenEdit(true);
  }
  ...
  function submitEdit(e) {
    e.preventDefault();
    if (!selectedLabtech) return;

    put(route('admin.labtechnicians.update', selectedLabtech.id), {
      onSuccess: () => {
        resetEdit();
        setOpenEdit(false);
      },
  ...
  function deleteLabtech(labtech) {
    if (!window.confirm('Are you sure you want to delete this lab technician? This cannot be undone.')) return;
    router.delete(route('admin.labtechnicians.destroy', labtech.id), {
      preserveScroll: true,
    });
  }
  ...
  function handleSearchChange(e) {
    ...
    router.get(route('admin.labtechnicians.index'), { search: e.target.value }, { preserveState: true, replace: true });
  }
              ...
              <Dialog open={openNew} onOpenChange={setOpenNew}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm">
                    <Plus className="mr-2 h-4 w-4" /> New Lab Technician
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>Create Lab Technician</DialogTitle>
                  </DialogHeader>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                {labtechnicians.map((labtech) => (
                  <tr key={labtech.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">{labtech.license_number}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{labtech.first_name} {labtech.last_name}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{labtech.user?.email || ''}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{labtech.phone_number}</td>
                    <td className="px-6 py-4 text-center space-x-2">
                      <Button variant="ghost" size="sm" title="View" onClick={() => openViewModal(labtech)} className="hover:text-blue-600">
                        <Eye className="h-5 w-5" />
                      </Button>
                      <Button variant="ghost" size="sm" title="Edit" onClick={() => openEditModal(labtech)} className="hover:text-green-600">
                        <Edit2 className="h-5 w-5" />
                      </Button>
                      <Button variant="ghost" size="sm" title="Delete" onClick={() => deleteLabtech(labtech)} className="hover:text-red-600">
              ...
              {selectedLabtech && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div><Label>First Name:</Label><p>{selectedLabtech.first_name}</p></div>
                    <div><Label>Last Name:</Label><p>{selectedLabtech.last_name}</p></div>
                    <div><Label>Email:</Label><p>{selectedLabtech.user?.email || ''}</p></div>
                    <div><Label>Username:</Label><p>{selectedLabtech.user?.name || ''}</p></div>
                    <div><Label>License Number:</Label><p>{selectedLabtech.license_number}</p></div>
                    <div><Label>Phone Number:</Label><p>{selectedLabtech.phone_number}</p></div>
                  </div>
                </div>
              )}
            </DialogContent>
